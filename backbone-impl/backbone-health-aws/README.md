...
`backbone-health-aws` provides **health checks** for use with AWS services such as S3, DynamoDb.
...
`backbone-health-aws` provides abstract base classes for health checks. To use them, create `@Produces` methods that return health check instances:
...
See: [examples/backbone-health-aws](../../examples/backbone-health-aws)
