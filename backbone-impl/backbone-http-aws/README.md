...
`backbone-http-aws` provides **SigV4-signed outbound HTTP transport** for AWS endpoints that require request signing.
...
`backbone-observability-aws` uses this transport for AMP and X-Ray exporters. Applications typically depend on that module rather than calling the transport directly.
