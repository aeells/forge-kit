...
`backbone-security` provides opinionated, reusable security primitives for Java / Quarkus services
...
`backbone-security` provides annotations (`@Secured` and `@AllowedServices`) that act as interceptor
...
See: [examples/backbone-security](../../examples/backbone-security)
...
## Relationship to Backbone Platform

`backbone-security` exposes shared security building blocks used by Backbone-based services.

The Backbone Platform extends this with:
