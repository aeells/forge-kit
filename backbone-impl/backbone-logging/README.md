...
`backbone-logging` provides **structured logging for Quarkus HTTP services**:
...
Add `backbone-logging-api` for `@LogMethodEntry` and `backbone-logging` for runtime filters and the interceptor:
...
Annotate methods with `@LogMethodEntry` from `backbone-logging-api`:
...
See: [examples/backbone-logging](../../examples/backbone-logging) for configuration and `@LogMethodEntry` examples.
